...
for index, row in task_events_df.iterrows():
    if index <= 1209:
        ...
        if int(row['iat']/10) <= 1 * 60:
            ...
            command_create = job_template.format(job_name=pod_name, sleep_time=duration, memory_req=memory_request, cpu_req=cpu_request, location=location, memory_req_byte=memory_request_byte)
            # else:
